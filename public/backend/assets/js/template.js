...
(function($) {
  ...
  $(function() {
    ...
    //     if (current === "") {
    //       //for root url
    //       if (element.attr('href').indexOf("{{ route('admin.dashboard') }}") !== -1) {
    ...
    function addActiveClass(element, current) {
          // Verificar se a URL atual corresponde à URL do link do elemento
          if (current !== "" && element.attr('href').indexOf(current) !== -1) {
              element.parents('.nav-item').last().addClass('active'); // Adicionar classe 'active' ao item do menu pai
              element.addClass('active'); // Adicionar classe 'active' ao próprio link
      
              // Se houver um submenu, mostrar o submenu
              if (element.parents('.sub-menu').length) {
                  element.closest('.collapse').addClass('show');
              }
      
              // Se o elemento estiver dentro de um submenu, adicionar a classe 'active' ao submenu
              if (element.parents('.submenu-item').length) {
                  element.addClass('active');
              }
          }
      ...
      // Iterar sobre os elementos de navegação
      $('.nav-link').each(function() {
          addActiveClass($(this), currentUrl); // Chamar a função addActiveClass para cada elemento de navegação
      });
